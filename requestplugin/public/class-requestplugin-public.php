...
class Requestplugin_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/requestplugin-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/requestplugin-public.js', array( 'jquery' ), $this->version, false );

	}

	static function api_request_get_content_generated() {
		...
		if ( isset( $_POST['subject'] ) && isset( $_POST['keyword'] ) ) {
			...
			$response = wp_remote_post( 'http://localhost/custom_admin_pages/wp-json/auto-content-generator/v1/send-inputs', array(
				'body' => array(
					'subject' => $subject,
					'keyword' => $keyword,
				),
			...
			<?php 
			$get_responsed_data = trim($responsed_data);
